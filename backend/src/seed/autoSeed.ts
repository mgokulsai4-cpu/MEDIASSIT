/**
 * Auto-seed function invoked during server startup when using in-memory MongoDB.
 * Re-uses the same seed data as the standalone seed script, but runs within
 * the server's existing database connection instead of creating a new one.
 */
...
export async function autoSeed(): Promise<void> {
  ...
  for (const d of SEED_DOCTORS) {
    ...
    await Doctor.create({ ...d, doctor_id: d.code, availability: buildDaySlots(), status: 'available' });
  }
  logger.info('Doctors seeded');

  // Seed doctor user accounts (password: Doctor@123)
  ...
  // Seed demo patient account (password: Patient@123)
